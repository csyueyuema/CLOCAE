#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Statement-level 5-fold CV with fixed data (no leakage)."""
...
def eval_model(model_dir, test_file, preds_file):
    ...
    test_loader = DataLoader(test_ds, batch_size=8, shuffle=False,
        collate_fn=lambda b: collate_fn(b, tokenizer, args.get('max_length', 256), True))
    device = torch.device('cuda')
    model = WideDeepRanker(model_name=str(model_dir/'encoder'), wide_dim=len(feature_keys),
        deep_hidden=args.get('deep_hidden', 256), cae_gate=True, cae_start_idx=7).to(device)
...
def main():
    output_dir = ROOT / 'ranker' / 'results' / 'kfold_line_gate_checked'
    output_dir.mkdir(parents=True, exist_ok=True)
    data_dir = ROOT / 'ranker' / 'data' / 'manual_line_stage3_cae_folded'

    all_fold_results = {m: [] for m in ['CrashLocCAE','LLM-Only','CrashLocator','Scaffle','w/o CAE Features']}

    # Baselines (file-level scores, same for all folds)
    ...
    for r in all_file_rows:
        if (r['bug_id'], r['file']) not in cl_scores:
            cl_scores[(r['bug_id'], r['file'])] = cl.score(r)
    ...
    for fold_idx in range(5):
        ...
        out_fold = output_dir / f'fold_{fold_idx}'
        ...
        print(f'\n{"="*60}')
        print(f'FOLD {fold_idx+1}/5: {len(test_pos_bugs)} pos bugs')
        print(f'{"="*60}')

        # Train gated model
        model_dir = out_fold / 'model'
        ...
        preds_file = out_fold / 'preds.jsonl'
        if not preds_file.exists():
            print('--- Evaluate ---')
            m = eval_model(model_dir, test_file, preds_file)
            print(f'  gate: t1={m["top1"]} t10={m["top10"]} mar={m["mar"]:.1f}')

        # w/o CAE
        no_cae_model = out_fold / 'model_no_cae'
        no_cae_preds = out_fold / 'preds_no_cae.jsonl'
        if not (no_cae_model / 'ranker.pt').exists():
            ...
            strip_cae(train_file, out_fold / 'train_no_cae.jsonl')
            strip_cae(test_file, out_fold / 'test_no_cae.jsonl')
            subprocess.run([PYTHON, 'ranker/train_ranker_paper.py',
                '--train-file', str(out_fold / 'train_no_cae.jsonl'),
                '--valid-file', str(out_fold / 'test_no_cae.jsonl'),
        ...
        gate_preds = read_jsonl(preds_file) if preds_file.exists() else []
    ...
    # Summary
    print(f'\n{"="*80}')
    print('STATEMENT-LEVEL 5-FOLD CV (FIXED, no leakage)')
    print(f'{"="*80}')
    summary = {}

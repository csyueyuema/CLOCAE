...
from train_line_ranker_current import LINE_FEATURE_KEYS, LineRankingDataset, WideDeepLineRankerV2, collate_fn
from train_ranker_current import FeatureNormalizer, compute_ranking_metrics
...
def main():
    ...
    parser.add_argument('--model-dir', default='ranker/outputs/line_ranker')
    parser.add_argument('--test-file', default='ranker/data/manual_line_stage3_light/test.jsonl')
    parser.add_argument('--output-file', default='ranker/outputs/line_ranker/test_metrics.json')
    ...
    test_ds = LineRankingDataset(Path(args.test_file).resolve(), feature_keys=feature_keys, normalizer=normalizer)

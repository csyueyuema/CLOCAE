#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Wide & Deep ranker with Focal Loss + CAE feature gating.

Changes from paper-faithful implementation:
1. Focal Loss (alpha=0.25, gamma=2.0) instead of BCE
2. CAE feature gate: when all CAE features are 0, Wide branch is automatically downweighted
"""
...
class WideDeepRanker(nn.Module):
    def __init__(self, model_name: str, wide_dim: int, deep_hidden: int = 256,
                 use_wide: bool = True, use_deep: bool = True, cae_gate: bool = True,
                 cae_start_idx: int = 7):
        super().__init__()
        self.use_wide = use_wide
        self.use_deep = use_deep
        self.cae_gate = cae_gate
        self.cae_start_idx = cae_start_idx
        hidden_parts = []
    ...
    def forward(self, input_ids, attention_mask, wide_features):
        ...
        if self.use_wide:
            ...
            if self.cae_gate and self.cae_start_idx < wide_features.shape[1]:
                cae_feats = wide_features[:, self.cae_start_idx:]
                cae_signal = (cae_feats.abs() > 1e-6).float().sum(dim=1, keepdim=True)
                gate = torch.sigmoid(cae_signal - 0.5)
                wide_out = wide_out * gate
...
def main():
    ...
    parser.add_argument('--no-cae-gate', action='store_true')
    ...
    cae_start_idx = 7  # first 7 are stack+structure, rest are CAE
    ...
    model = WideDeepRanker(
        ...
        use_wide=use_wide,
        use_deep=use_deep,
        cae_gate=not args.no_cae_gate,
        cae_start_idx=cae_start_idx,
    ).to(device)
